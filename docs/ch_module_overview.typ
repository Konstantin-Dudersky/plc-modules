...
- Количеством микросхем, которыми должен управлять микроконтроллер. На "простых" платах (например, #link(<PMCNV-DIx16>)[PMCNV-DIx16] или #link(<PMCNV-RQx8>)[PMCNV-RQx8]) такая микросхема одна. На более "сложных" может быть больше. Например, на плате #link(<PMCNV-AI4T>)[PMCNV-AI4T] таких микросхем 3.
- Мощностью, потребляемой компонентами. Например, на плате #link(<PMCNV-RQx8>)[PMCNV-RQx8]) установлены реле, которые потребляют значительный ток, поэтому количество таких плат ограничено 2.

Например, к плате #link(<PMCNV-DIx16>)[PMCNV-DIx16] можно подключить 16 дискретных датчиков. T.е. к модулю можно подключить до 64 дискретных датчиков.
