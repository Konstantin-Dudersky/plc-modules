...
- Количеством микросхем, которыми должен управлять микроконтроллер. На "простых" платах (например, #link(<PM_CNV-DI16_sink>)[PM_CNV-DI16_sink] или #link(<PM_CNV-RQ8>)[PM_CNV-RQ8]) такая микросхема одна. На более "сложных" может быть больше. Например, на плате #link(<PM_CNV-AI4_TC>)[PM_CNV-AI4_TC] таких микросхем 3.
- Мощностью, потребляемой компонентами. Например, на плате #link(<PM_CNV-RQ8>)[PM_CNV-RQ8]) установлены реле, которые потребляют значительный ток, поэтому количество таких плат ограничено 2.

Например, к плате #link(<PM_CNV-DI16_sink>)[PM_CNV-DI16_sink] можно подключить 16 дискретных датчиков. T.е. к модулю можно подключить до 64 дискретных датчиков.
