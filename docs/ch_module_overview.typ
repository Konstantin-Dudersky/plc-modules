...
- Количеством микросхем, которыми должен управлять микроконтроллер. На "простых" платах (например, #link(<PMCNV-DI16>)[PMCNV-DI16sink] или #link(<PMCNV-RQ8>)[PM-CNV_RQ8]) такая микросхема одна. На более "сложных" может быть больше. Например, на плате #link(<PMCNV-AI4T>)[PMCNV-AI4T] таких микросхем 3.
...
Например, к плате #link(<PMCNV-DI16>)[PMCNV-DI16sink] можно подключить 16 дискретных датчиков. T.е. к модулю можно подключить до 64 дискретных датчиков.
