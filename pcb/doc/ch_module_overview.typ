...
- Количеством микросхем, которыми должен управлять микроконтроллер. На "простых" платах (например, #link(<PMCNV-DIx32>)[PMCNV-DIx32] или #link(<PMCNV-RQx8>)[PMCNV-RQx8]) такая микросхема одна. На более "сложных" может быть больше. Например, на плате #link(<PMCNV-AI8T>)[PMCNV-AI8T] таких микросхем 3.
...
Например, к плате #link(<PMCNV-DIx32>)[PMCNV-DIx32] можно подключить 32 дискретных датчиков. T.е. к модулю можно подключить до 32 дискретных датчиков.
