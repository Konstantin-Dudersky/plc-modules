#import "../doc/functions.typ": calc_power_consumtion, table_power_consumtion
#import "../doc/load_pcb_data.typ": load_pcb_data
#import "../doc/table_options.typ": bom_options, table_options

#let article = "PMCNV-DIx32"
#let version = "0.1.0"

#pagebreak()

== #article - 32 дискретных входов <PMCNV-DIx32>
...
Датчики можно подключать по схеме PNP или NPN. Конкретный тип выбирается с помощью перемычки.

Функциональная схема платы представлена на @PMCNV-DI16__function[рисунке].

#figure(
  caption: "Функциональная схема платы",
  image("../doc/images/plates_function_scheme/PM_CNV-DI16.svg", width: 100%),
) <PMCNV-DI16__function>
...
#figure(
  ...
  image("../doc/images/CA-IS3980P_channel_circuit.png", width: 100%),
...
#figure(
  ...
  table(
    ...
    table.header(
      repeat: true,
      [*Тип согласно IEC 61131-2*], [*$R_1$*], [*$R_2$*],
    ),
...
На плате есть два разъема FFC, через которые подключается плата светодиодов #link(<PMLED-18>)[PMLED-18].
...
#figure(
  ...
  image("../doc/images/DI16_расчет.svg", height: 200pt),
) <DI16_расчет>
...
#figure(
  ...
  table(
    ...
    table.header(
      repeat: true,
      [Напряжение, [В]], [Порог, [В]], [$R_1$, [Ом]], [$R_2$, [Ом]],
    ),
...
#let power_consumption = (
  CA-IS3980P: (2, 25.41, 0),
  "MCP23S17-E/SO": (1, 3.3, 0),
  "Светодиоды": (18, 9.9, 0),
)
#let pmcnv_di16_power = calc_power_consumtion(power_consumption)
#table_power_consumtion(values: pmcnv_di16_power)

=== Опции

#let options = (
  (
    desc: "Количество дискретных входов",
    choices: (
      (name: "8", desc: "8 каналов", price: 777),
      (name: "16", desc: "16 каналов", price: 777),
      (name: "24", desc: "24 канала", price: 777),
      (name: "32", desc: "32 канала", price: 777),
    ),
  ),
  (
    desc: "Схема подключения входов",
    choices: (
      (name: "NPN", desc: "Схема подключения NPN", price: 0),
      (name: "PNP", desc: "Схема подключения PNP", price: 0),
    ),
  ),
  (
